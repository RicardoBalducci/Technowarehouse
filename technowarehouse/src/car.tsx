...
function ShoppingCart() {
    ...
    const addToCart = (product: Product) => {
        ...
        if (existingItem) {
            ...
            const cartItem: CartItem = { ...product, cartItemId: Math.random(), quantity: 1 };
            ...
            <div className='container mt-5'>
                <div className='row'>
                        ...
                        <ul>
                            {cartItems.map((item, index) => (
                                <li className='product p-2 m-2 ' key={item.cartItemId}>
                                    ...
                                    <div className='iz'>
                                    <button className='btn-p  m-1' onClick={() => decreaseQuantity(item.cartItemId)}>-</button>
                                    <div>
                                    <h5 className='m-1 p-2'>{item.quantity}</h5>
                                    </div>

                                    <button className='btn-p  text-light m-1' onClick={() => increaseQuantity(item.cartItemId)}>+</button>
                                    <button className='btn-eliminar   m-1' onClick={() => removeFromCart(item.cartItemId)}></button>
                                    </div>
                                </li>
                            ))}
                        </ul>
                    ...
                    <div className=' sticky-top col-md-6  container d-flex justify-content-center align-items-center'>
                        ...
                        <p>Total a pagar:  <h2>${getTotalPrice()}</h2></p>
